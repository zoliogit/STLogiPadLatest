...
-(void)getfav:(NSString*)username
{
    ...
    NSString *querySelect = [NSString stringWithFormat:@"SELECT a.products_desc,a.products_uom,a.products_qty,a.products_id from products_details a ,favourites b WHERE a.products_id = b.products_id AND b.user_id = '%@'",username];
    query_stmt_Select = [querySelect UTF8String];
    if (sqlite3_prepare_v2(database, query_stmt_Select, -1, &selectStmt, NULL) == SQLITE_OK)
    {
        while (sqlite3_step(selectStmt) == SQLITE_ROW)
